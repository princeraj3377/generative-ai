...
db.run("""
    CREATE TABLE IF NOT EXISTS tasks (
        ...
        title TEXT NOT NULL,
        description TEXT,
        status TEXT CHECK(status IN ('pending','in_process','completed')) DEFAULT 'pending',
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    );
""")


## llm ,tools ,memory ,sysytem_rpomt 
...
@st.cache_resource
def get_agent():
    agent = create_agent(model= model,tools=tools,checkpointer= InMemorySaver(),system_prompt=system_prompt)
    return agent


agent = get_agent()

st.subheader("📜  My task application ")
if "messages" not in st.session_state:
    st.session_state.messages=[]



for messaages in st.session_state.messages:
    st.chat_message(messaages["role"]).markdown(messaages["content"])

    


prompt = st.chat_input("enter your task")
if prompt:
